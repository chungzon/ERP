...
import ERP.Model.ProductWaitingConfirm.SingleProductUpdate.UnitechGetWebInfo_Model;
...
/** [Controller] Unitech login */
public class UnitechLogin_Controller {
    ...
    public UnitechLogin_Controller(){
        KeyPressed = ERPApplication.ToolKit.KeyPressed;
        ComponentToolKit = ERPApplication.ToolKit.ComponentToolKit;
        UnitechGetWebInfo_Model = ERPApplication.ToolKit.ModelToolKit.getUnitechGetWebInfoModel();
    }
    ...
    public void setLoginInfo() throws Exception{
        ...
        Image Image = UnitechGetWebInfo_Model.getLoginCaptcha();
    ...
    @FXML protected void LoginKeyPressed(KeyEvent KeyEvent) throws Exception{
        if(KeyPressed.isEnterKeyPressed(KeyEvent)){
            CaptchaError.setText("");
            HashMap<Boolean, Map<String,String>> LoginCookieMap = UnitechGetWebInfo_Model.Login(CompanyTaxText.getText(), AccountText.getText(), PasswordText.getText(), CaptchaText.getText(), CaptchaError);
            if (LoginCookieMap.containsKey(true)) {
                SingleProductUpdate_Model.UpdateVendorCookie(Product_Enum.Vendor.精技電腦.getEnglishName(), LoginCookieMap.get(true).toString());
                UpdateStatus.setText("精技登入成功，請重新執行!");
                ERPApplication.Logger.info("[單筆更新] 精技 Cookie 過期，登入成功!");
                ComponentToolKit.closeThisStage(Stage);
            }else{
                UpdateStatus.setText("精技登入失敗!");
                ERPApplication.Logger.info("[單筆更新] 精技 Cookie 過期，登入失敗!");
            }
        }
    }

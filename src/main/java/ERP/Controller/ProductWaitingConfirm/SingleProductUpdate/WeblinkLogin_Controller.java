...
import ERP.Model.ProductWaitingConfirm.SingleProductUpdate.WeblinkGetWebInfo_Model;
...
/** [Controller] Weblink login */
public class WeblinkLogin_Controller {
    ...
    @FXML private TextField  AccountText, CaptchaText;
    @FXML private PasswordField PasswordText;
    @FXML private ImageView ImageView;
    @FXML private Label CaptchaError;

    public WeblinkLogin_Controller(){
        KeyPressed = ERPApplication.ToolKit.KeyPressed;
        ComponentToolKit = ERPApplication.ToolKit.ComponentToolKit;
        WeblinkGetWebInfo_Model = ERPApplication.ToolKit.ModelToolKit.getWeblinkGetWebInfoModel();
    }
    ...
    public void setLoginInfo() throws Exception{
        ...
        Image Image = WeblinkGetWebInfo_Model.getLoginCaptcha();
    ...
    @FXML protected void LoginKeyPressed(KeyEvent KeyEvent) throws Exception{
        if(KeyPressed.isEnterKeyPressed(KeyEvent)){
            CaptchaError.setText("");
            HashMap<Boolean, Map<String,String>> LoginCookieMap = WeblinkGetWebInfo_Model.Login(AccountText.getText(), PasswordText.getText(), CaptchaText.getText(), CaptchaError);
            if (LoginCookieMap.containsKey(true)) {
                SingleProductUpdate_Model.UpdateVendorCookie(Product_Enum.Vendor.展碁國際.getEnglishName(), LoginCookieMap.get(true).toString());
                UpdateStatus.setText("展碁登入成功，請重新執行!");
                ERPApplication.Logger.info("[單筆更新] 展碁 Cookie 過期，登入成功!");
                ComponentToolKit.closeThisStage(Stage);
            }else{
                UpdateStatus.setText("展碁登入失敗!");
                ERPApplication.Logger.info("[單筆更新] 展碁 Cookie 過期，登入失敗!");
            }
        }
    }
